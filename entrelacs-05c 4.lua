...
-- 7 notes max
-- fixer nbr de caractères max pour 'name' -> 16 max
xenutil.SCALES = {
{name = "Derives Minor 31", intervals = {0, 2.3226, 3.0968, 5.0323, 6.9677, 8.1290, 10.0645}, interv_name = {'R','M2','m3','P4','P5','m6','m7'}}, -- 7 éléments
{name = "Harm Minor 31", intervals = {0, 2.3226, 3.0968, 5.0323, 6.9677, 8.1290, 10.8387}, interv_name = {'R','M2','m3','P4','P5','m6','M7'}}, -- 7 éléments
{name = "Harm Major 31", intervals = {0, 2.3226, 3.8710, 5.0323, 6.9677, 8.9032, 10.8387}, interv_name = {'R','M2','M3','P4','P5','M6','M7'}}, -- 7 éléments
...
function play_jf(state1)
    ...
    if type1 == 'number' and solofond == false then -- vérifie si 'ss1' retourne bien un nombre -> le sequins retourne une valeur
        ...
        crow.output[1].volts = notejf + params:get("octaveRings")
    elseif type1 == 'number' and solofond == true then
        notejf = (ss1/12) + params:get("octave1") + (params:get("semitone")/12) + (params:get("cents")/12)
        crow.ii.jf.play_note(notejf, 5)
        crow.output[1].volts = params:get("octave1") + (params:get("semitone")/12) + (params:get("cents")/12) + params:get("octaveRings") -- Rings joue la note fondamentale uniquement
...
function play_wsyn(s)
    ...
    if type2 == 'number' then -- vérifie si 'ss1' retourne bien un nombre -> le sequins retourne une valeur
        ...
        crow.ii.wsyn[1].play_note(notewsyn, 0.5)
...
function play_wsyn2(s)
    ...
    if type3 == 'number' then -- vérifie si 'ss1' retourne bien un nombre -> le sequins retourne une valeur
        ...
        crow.ii.wsyn[2].play_note(notewsyn2, 0.5)
...
function play_out1(s) -- canal 4 voix Beads
    ...
    if type4 == 'number' and mutevoct4 == false then -- vérifie si 'ss1' retourne bien un nombre -> le sequins retourne une valeur
        note_out1 = (ss4/12) + params:get("octave4") + (params:get("cents")/12)
